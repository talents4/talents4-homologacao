/* Empacota a V2.5 isolada. Nunca faz push, deploy, SQL, backup ou acesso ao Supabase. */
...
const output = fs.mkdtempSync(path.join(parent, 'talents4-v25-entrega-'));
const stage = fs.mkdtempSync(path.join(parent, 'talents4-v25-stage-'));
...
const preview = path.join(stage, 'preview-talentos-v2-5');
...
fs.writeFileSync(path.join(preview, 'LEIA_ME.md'), `# Talents 4 V2.5 · prévia sem banco
...
const previewZip = path.join(output, 'Talents4_V2_5_PREVIA_SEM_BANCO.zip');
run('zip', ['-q', '-r', previewZip, 'preview-talentos-v2-5'], stage);
run('unzip', ['-tq', previewZip]);

const fullZip = path.join(output, 'Talents4_V2_5_CODIGO_COMPLETO.zip');
run('git', ['archive', '--format=zip', `--output=${fullZip}`, 'HEAD']);
run('unzip', ['-tq', fullZip]);

const incremental = path.join(stage, 'talents4-v2-5-incremental');
...
const incrementalZip = path.join(output, 'Talents4_V2_5_ATUALIZACAO_INCREMENTAL.zip');
run('zip', ['-q', '-r', incrementalZip, 'talents4-v2-5-incremental'], stage);
...
fs.writeFileSync(path.join(output, 'VERIFICACAO_LOCAL.txt'), 'Talents 4 V2.5 · homologação isolada\nSem push, deploy, SQL, Google Planilhas, Google Drive, backup ou alteração de banco.\n');
