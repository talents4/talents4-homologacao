...
check(modelsCode.includes("if (/nao gost/.test(status)) return 'sent';") && modelsCode.includes('excluid[oa]') && organizationCode.includes('Histórico de excluídos e removidos') && organizationCode.includes("const analyticalPanels = display === 'list' && scope !== 'closed' ? openPanel + hiredPanel : ''") && organizationCode.includes('Todas as seleções em aberto') && organizationCode.includes('Contratações mais recentes') && organizationCode.includes('org-selection-stage-groups') && organizationCode.includes('data-selection-stage-group') && organizationCode.includes("label: 'Em análise'") && organizationCode.includes("label: 'Apresentados'") && organizationCode.includes("label: 'Entrevistas'") && organizationCode.includes("label: 'Propostas'") && organizationCode.includes('(scope ===') && organizationCode.includes('historySection)') && organizationCode.includes("const current = display === 'cards' && scope !== 'closed'") && organizationCode.includes('changes.is_active') && v25CSS.includes('.org-selection-stage-groups') && v25CSS.includes('.org-selection-stage-cards'), 'Seleções mantêm painéis na Lista analítica e separam abertos por etapa; o Kanban fica isolado');
