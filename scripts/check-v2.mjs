...
check(modelsCode.includes("if (/nao gost/.test(status)) return 'sent';") && modelsCode.includes('excluid[oa]') && organizationCode.includes('Histórico de excluídos e removidos') && !organizationCode.includes('Todas as seleções em aberto') && !organizationCode.includes('Contratações mais recentes') && organizationCode.includes('changes.is_active'), 'Seleções preservam Não gostou, isolam excluídos/removidos e sincronizam cancelamento de vaga');
