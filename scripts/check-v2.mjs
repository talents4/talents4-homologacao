...
check(modelsCode.includes("if (/nao gost/.test(status)) return 'sent';") && modelsCode.includes('excluid[oa]') && organizationCode.includes('Histórico de excluídos e removidos') && organizationCode.includes("const analyticalPanels = display === 'list' ? openPanel + hiredPanel : ''") && organizationCode.includes('Todas as seleções em aberto') && organizationCode.includes('Contratações mais recentes') && organizationCode.includes(' + analyticalPanels + current + historySection +') && organizationCode.includes('changes.is_active'), 'Seleções mantêm painéis na Lista analítica, isolam o Kanban e sincronizam cancelamento de vaga');
