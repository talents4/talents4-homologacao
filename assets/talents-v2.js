(function () {
  ...
  function selectionBar(rows) {
    ...
    const selectionText = selected ? `${selected} Talento${selected === 1 ? '' : 's'} selecionado${selected === 1 ? '' : 's'}` : 'Selecione Talentos para gerar os dois modelos oficiais.';
    return `<div class="t4-bulk-bar ${selected ? 'has-selection' : ''}" role="region" aria-label="Seleção para exportação"><div><span class="t4-bulk-icon">${U.icon(selected ? 'check' : 'list')}</span><strong>${e(selectionText)}</strong><small>${selected ? 'A seleção fica preservada enquanto você troca de visão ou aplica filtros.' : 'A seleção não altera etapas nem libera apresentação automaticamente.'}</small></div><div class="t4-bulk-actions">${W.button(allVisible ? 'Limpar recorte' : 'Selecionar recorte', allVisible ? 'deselect-visible' : 'select-visible', '', { className: 'ghost sm', icon: allVisible ? 'close' : 'check' })}${selected ? W.button('Limpar seleção', 'clear-selection', '', { className: 'ghost sm' }) : ''}${W.button(selected ? 'Exportar seleção' : 'Exportar planilhas', 'data-center', '', { className: selected ? 'primary sm' : 'sm', icon: 'download' })}</div></div>`;
  }
  ...
  const PIPELINE_STAGES = [
    { label: 'Novo candidato' }, { label: 'Triagem' }, { label: 'Pré-seleção' }, { label: 'Análise' },
    { label: 'Curso de Alemão', tone: 'info' }, { label: 'Entrevista', tone: 'info' }, { label: 'Documentação', tone: 'warning' },
  ...
  function overview() {
    ...
    return `<div class="t4-work-intro"><div><span class="t4-overline">SEU ESPAÇO DE TRABALHO</span><h2>Informação clara. Próximo passo definido.</h2><p>Bom trabalho, ${e(D.profile?.nome || 'equipe')}. Aqui está o que precisa avançar.</p></div><span class="t4-date-chip">${U.icon('calendar')}${e(new Date().toLocaleDateString('pt-BR', { day: 'numeric', month: 'long' }))}</span></div>
      ...
      <div class="t4-shortcuts">${W.button('Consultar oportunidades', 'go', 'opportunities', { icon: 'briefcase' })}${W.button('Exportar planilhas', 'data-center', '', { icon: 'download' })}${W.link('Planejamento dos empregadores', './organizacional.html?view=planning', 'building')}${W.link('Acompanhamento de alemão', './alemao.html?view=attention', 'graduation')}${W.link('Agenda de contatos', './contatos.html', 'contact')}</div>`;
  }
