(function () {
  ...
  const state = { talents: [], employers: [], openings: [], selections: { rows: [], modern: false }, activities: [], plans: [], meetings: [], summaries: [], replacements: [], tasks: [], metrics: [], query: '', employer: '', month: '', status: '', display: 'list', calendar: M.today().slice(0, 7), loaded: false, archive: null };
  ...
  function opportunityRegister(rows) {
    ...
    const groups = new Map(); rows.forEach((r) => { const key = String(r.employer_id || 'internal'); if (!groups.has(key)) groups.set(key, []); groups.get(key).push(r); });
    return `<div class="mx-register mx-opportunity-register">${[...groups.entries()].map(([key, items]) => { const emp = W.find(state.employers,key) || {id:key,nome:employerOf(items[0])}, color=window.T4Modern?.color(emp)||'#7890a4'; return `<section class="mx-register-group" style="--employer-color:${a(color)}"><header><div>${window.T4Modern?.employer ? window.T4Modern.employer(emp) : `<strong>${e(emp.nome)}</strong>`}<span>${items.length} oportunidade(s) · ${items.reduce((n,r)=>n + (Number(r.quantity)||0),0)} posição(ões)</span></div><button type="button" class="t4-btn ghost sm" data-action="employer-detail" data-id="${a(key)}">Dossiê</button></header><div class="mx-register-rows">${items.map((r) => { const linked=state.selections.rows.filter(s=>M.same(s.opening_id,r.id)); return `<article class="mx-register-row"><div><button class="t4-row-link" data-action="opening-detail" data-id="${a(r.id)}">${e(r.title || 'Oportunidade sem nome')}</button><span>${e(r.area || 'Área não informada')}</span></div><div><strong>${e(String(r.quantity || 0))} posição(ões)</strong><span>${e(r.location || 'Local não informado')}</span></div><div>${W.status(r.status)}<span class="mx-register-meta">${e(r.language_requirement || 'Idioma não informado')}</span></div><div><strong>${linked.length}</strong><span>Talento(s) vinculado(s)</span></div><div class="mx-register-actions">${actions(r,'opening')}</div></article>`; }).join('')}</div></section>`; }).join('')}</div>`;
  }
