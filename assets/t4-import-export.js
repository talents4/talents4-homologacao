...
(function (global) {
  ...
  function parseBook(book) {
    ...
    const kind = names.has('candidatos priorizados') || names.has('nectanet partner') || names.has('empresas detalhadas') ? 'nectanet'
      : names.has('resumo bw') || names.has('radar nectanet') || (book.sheets || []).some((sheet) => headerRow(sheet, trackingFields)) ? 'mapping' : 'unknown';
    const result = { name: book.name, kind, presentation: [], partners: [], companies: [], tracking: [], contexts: [], summaries: [], radar: [], unknownSheets: [] };
  ...
  function parseBooks(books) {
    ...
    const nectanet = parsed.find((book) => book.kind === 'nectanet') || { presentation: [], partners: [], companies: [], unknownSheets: [] };
    const mapping = parsed.find((book) => book.kind === 'mapping') || { tracking: [], contexts: [], summaries: [], radar: [], unknownSheets: [] };
    ...
    return { books: parsed, nectanet, mapping, talentNames: names, warnings,
      stats: { files: parsed.length, candidates: nectanet.presentation.length, partners: nectanet.partners.length, companies: nectanet.companies.length, tracking: mapping.tracking.length, profiles: mapping.contexts.length, summary: mapping.summaries.length, radar: mapping.radar.length } };
  }
  ...
  async function importData(source, options = {}) {
    ...
    const registerEmployer = (raw, patch = {}) => {
    ...
    companyRows.forEach((row) => registerEmployer(row.empresa, { area_atuacao: row.sector, perfis_buscados: row.openings, descricao_resumida: row.description, email_principal: row.send_email }));
    (source.nectanet.partners || []).forEach((row) => registerEmployer(row.empresa, {}));
    (source.nectanet.presentation || []).forEach((row) => ['employer_primary', 'employer_alt1', 'employer_alt2'].forEach((key) => registerEmployer(row[key])));
    (source.mapping.tracking || []).forEach((row) => registerEmployer(row.empresa, { perfis_buscados: row.type_area }));
    (source.mapping.radar || []).forEach((row) => registerEmployer(row.empresa, { perfis_buscados: row.vacancy_situation }));
    ...
    return { imported: { talents: allNames.length, newTalents: newTalentRows.length, profiles: profileRows.length, partners: partnerRows.length, items: itemRows.length, employers: newEmployers.length }, removedColumns: [] };
  }
  ...
  function buildMappingWorkbook(state, people) {
    ...
    const radarHeader = T.FIELDS.radar.map(([, label]) => label);
    return { sheets: [
      ...
      { name: 'Radar NectaNet', rows: [['RADAR NECTANET — MATCHES RELEVANTES AOS TALENTOS'], [], radarHeader, ...radarRows], headerRow: 3, freezeRows: 3, widths: [38, 28, 17, 15, 15, 18, 58, 72, 28, 64, 26], merges: ['A1:K1'] }
    ] };
  }
  ...
  function open(options = {}) {
    const state = options.state || {}, load = options.load, selectedCount = state.selectedTalents instanceof Set ? state.selectedTalents.size : 0;
    let parsed = null, activeFiles = [], working = false;
    const modal = U.openModal({ title: 'Centro de dados', subtitle: 'Importe os modelos oficiais e exporte somente o recorte que você selecionou.', wide: true, body: '<div data-data-center></div>', footer: '<span class="t4-save-hint" data-data-status>Os arquivos são processados no navegador antes da confirmação.</span><button type="button" class="t4-btn" data-cancel>Fechar</button>' });
    ...
    const render = () => {
      ...
      body.innerHTML = parsed ? `<div class="t4-data-summary"><div><span class="t4-overline">PRÉVIA DA IMPORTAÇÃO</span><h3>${e(parsed.stats.candidates + parsed.stats.tracking)} linhas operacionais identificadas</h3><p>Revise a origem e confirme somente quando os números fizerem sentido. Nada foi gravado ainda.</p></div>${stat('Talentos no primeiro modelo', parsed.stats.candidates)}${stat('Linhas por Talento', parsed.stats.tracking)}${stat('Empresas detalhadas', parsed.stats.companies)}${stat('Parceiros NectaNet', parsed.stats.partners)}${stat('Resumo + radar', `${parsed.stats.summary} + ${parsed.stats.radar}`)}</div><div class="t4-data-files"><div><strong>Arquivos reconhecidos</strong>${parsed.books.map((book) => `<span>${U.icon(book.kind === 'nectanet' ? 'note' : 'list')}<b>${e(book.name)}</b><small>${book.kind === 'nectanet' ? 'NectaNet + priorização + empresas' : 'Acompanhamento por abas + resumo + radar'}</small></span>`).join('')}</div><div><strong>Talentos encontrados</strong><p class="t4-preserve">${e(parsed.talentNames.slice(0, 12).join(' · '))}${parsed.talentNames.length > 12 ? ` · +${parsed.talentNames.length - 12}` : ''}</p></div></div>${parsed.warnings.length ? `<div class="t4-data-warnings">${parsed.warnings.map((warning) => W.note(warning, 'warning')).join('')}</div>` : W.note('Os campos serão distribuídos entre cadastro único, contexto de apresentação, acompanhamento e empregadores. Resumo, radar e dados de parceiros serão preservados para a próxima exportação.', 'success')}<div class="t4-data-actions"><button type="button" class="t4-btn" data-data-reset>Escolher outros arquivos</button><button type="button" class="t4-btn primary" data-data-import ${working ? 'disabled' : ''}>${working ? 'Importando…' : 'Confirmar importação'}</button></div>`
      : `<div class="t4-data-hero"><span class="t4-overline">OPERAÇÃO DE DADOS</span><h3>Planilhas sob controle da equipe.</h3><p>Escolha os dois arquivos oficiais. O sistema identifica as abas, mostra uma prévia e só grava depois da sua confirmação.</p><label class="t4-dropzone"><input type="file" accept=".xlsx,.xlsm,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel.sheet.macroEnabled.12" multiple data-data-files><span>${U.icon('upload')}<strong>Selecionar os dois arquivos Excel</strong><small>Você pode selecionar os dois de uma vez · .xlsx ou .xlsm</small></span></label></div><div class="t4-data-cards">${card('Importar os dois modelos', 'Cadastro, contexto, parceiros, empresas e linhas de acompanhamento entram em uma prévia antes da gravação.', '<span class="t4-data-card-badge">Passo 1</span>', 'import')}${card('Exportar o recorte oficial', `${e(selectionCopy)}. Gera os dois modelos com abas por Talento, campos preservados e quebra de página.`, `<button type="button" class="t4-btn primary" data-data-export="both">Exportar os dois arquivos</button>`, 'export')}</div><div class="t4-data-export-grid"><div><strong>Exportações individuais</strong><p>Use quando precisar enviar apenas um dos modelos.</p><div><button type="button" class="t4-btn sm" data-data-export="nectanet">Somente NectaNet</button><button type="button" class="t4-btn sm" data-data-export="mapping">Somente acompanhamento</button></div></div><div><strong>Seleção atual</strong><p>${e(selectionCopy)}</p></div></div>`;
      bind();
    };
    const bind = () => {
      modal.querySelector('[data-cancel]')?.addEventListener('click', U.closeModal);
      modal.querySelector('[data-data-files]')?.addEventListener('change', async (event) => {
        activeFiles = [...event.target.files]; if (!activeFiles.length) return;
        setStatus('Lendo abas e fórmulas como valores exibidos…');
        try { parsed = parseBooks(await workbook().readMany(activeFiles)); setStatus('Prévia pronta. Revise antes de confirmar.', 'success'); render(); }
        catch (error) { activeFiles = []; setStatus('Não foi possível ler a planilha.', 'error'); body.innerHTML = W.note(error.message || 'Arquivo inválido.', 'error') + '<div class="t4-data-actions"><button type="button" class="t4-btn" data-data-reset>Escolher novamente</button></div>'; bind(); }
      });
      modal.querySelector('[data-data-reset]')?.addEventListener('click', () => { parsed = null; activeFiles = []; setStatus('Os arquivos são processados no navegador antes da confirmação.'); render(); });
      modal.querySelector('[data-data-import]')?.addEventListener('click', async () => {
        if (!parsed || working) return; working = true; render();
        try {
          await importData(parsed, { state, progress: (label, done, total) => setStatus(`${label}: ${done} de ${total || done}`) });
          setStatus('Importação concluída. Atualizando o CRM…', 'success'); U.toast('Planilhas importadas com os campos distribuídos no CRM.', 'success'); await load?.(); parsed = null; activeFiles = []; working = false; render();
        } catch (error) { working = false; setStatus('A importação foi interrompida; confira a mensagem abaixo.', 'error'); render(); body.insertAdjacentHTML('afterbegin', W.note(U.formatError(error), 'error')); }
      });
      modal.querySelectorAll('[data-data-export]').forEach((button) => button.addEventListener('click', async () => {
        try { const count = await exportFiles(state, button.dataset.dataExport); U.toast(`${count} Talento${count === 1 ? '' : 's'} exportado${count === 1 ? '' : 's'} nos modelos oficiais.`, 'success'); setStatus('Exportação concluída. Confira os downloads do navegador.', 'success'); }
