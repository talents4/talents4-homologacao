  ...
  function editSelection(state, row, context = {}, after) {
    ...
    if (!modern) {
      ...
      return W.recordForm({ title: 'Editar vínculo geral', subtitle: talentName(state, row.talent_id) + ' · ' + employerName(state, row.employer_id), row: original, table: row._source,
        ...
        fields: [
          { name: 'status_vinculo', label: 'Etapa do vínculo', required: true, type: 'select', options: ['Aguardando envio', 'Aguardando resposta', 'Reunião marcada', 'Em processo', 'Gostou', 'Não gostou', 'Contratado', 'Removido'], placeholder: null },
