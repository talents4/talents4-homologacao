...
(function () {
  ...
  function create({ state, app, load, render, talentDetail }) {
    ...
    function sheetTabs(current) {
      return `<nav class="tw-sheet-tabs" aria-label="Visões do mapeamento">${[['talents','Base de talentos'],['mapping','Acompanhamento'],['presentation','Prontos para apresentar'],['mapping-summary','Resumo BW'],['mapping-radar','Radar NectaNet']].map(([id,label]) => `<button type="button" data-action="go" data-id="${id}" ${id === current ? 'aria-current="page" class="current"' : ''}>${e(label)}</button>`).join('')}</nav>`;
    }
    ...
    function radar() {
      ...
      return sheetTabs('mapping-radar') + quickFilters() + toolbar() + trackingTable(rows,true);
    }
