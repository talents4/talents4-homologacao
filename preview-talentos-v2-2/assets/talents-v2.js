(function () {
  ...
  const app = U.mount({ module: 'talents', moduleLabel: 'Talentos', defaultView: 'talents', views: [
    ...
    { id: 'mapping', label: 'Acompanhamento', subtitle: 'Empresa por empresa: aderência, viabilidade e próximos alvos de cada Talento.', icon: 'list' },
    { id: 'presentation', label: 'Prontos para apresentar', subtitle: 'As 18 informações da apresentação Nectanet, na ordem da planilha.', icon: 'check' },
    { id: 'mapping-summary', label: 'Resumo BW', subtitle: 'Leitura consolidada do acompanhamento, com critérios explícitos.', icon: 'dashboard' },
    { id: 'mapping-radar', label: 'Radar NectaNet', subtitle: 'Alvos NectaNet e avaliações individuais, sem misturar pessoas ou scores.', icon: 'briefcase' },
  ...
  const state = { talents: [], employers: [], openings: [], selections: { rows: [], modern: false }, activities: [], enrollments: [], classes: [], mappingProfiles: [], mappingItems: [], mappingPartners: [], presentationDetails: [], filters: {}, query: '', stage: '', german: '', employer: '', owner: '', quick: [], board: 'board', loaded: false, detail: null, detailTab: 'profile', detailVersion: 0 };
  const sources = {
  ...
  function render() {
    ...
    const html = ({ overview, talents: () => directory(false), archived: () => directory(true), mapping: workspace.tracking, presentation: workspace.presentation, 'mapping-summary': workspace.summary, 'mapping-radar': workspace.radar, processes: selectionsView, opportunities: opportunitiesView, agenda: agendaView }[app.view] || overview)();
  ...
  function directory(archived) {
    ...
    return `${workspace.sheetTabs('talents')}${archived ? W.note('Nenhum talento é excluído por esta tela. A ficha completa, as seleções e o histórico de aulas continuam acessíveis.') : workspace.quickFilters()}${workspace.toolbar({archived})}${talentTable(filtered(archived))}`;
  }
  ...
  function opportunitiesView() {
    ...
    return workspace.workToolbar('openings') + W.table({ id: 'talent-openings', rows: list, columns: [
  ...
  app.onRoute(() => { state.multiOpen = ''; if (app.view === 'talents') state.quick = state.quick.filter((v) => v !== 'ready'); render(); });
  W.start(app, async () => {
    await load();
